...
class Board {
  constructor(canvas) {
    ...
    this.crossHair = new CrossHair(this.width, this.height);
  ...
  generateBombs() {
    ...
    window.setInterval(() => {
      if (!this.paused) {
        const newBomb = new Bomb(RandomStartPos(this.width), RandomEndPos(this.width,this.height), this.level);
  ...
  addLaser() {
    const newLaser = new Laser(this.mainCannon.endPos, this.crossHair.pos, this.level);
